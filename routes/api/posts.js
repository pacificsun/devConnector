...
try {
    //harek post ko id hunxa one user to many post relation. 
    const post = await Post.findById(req.params.id);
    // check if the post is already is liked by this user
    if(post.likes.filter(like => like.user.toString() === req.user.id).length > 0){
        //like.user ObjectId hunnxa teslai string ma lane
